...
function kama_excerpt( $args = '' ){
    ...
    if( ! $rg->ignore_more && strpos( $text, '<!--more-->' ) ){
        ...
        $text_append = sprintf( ' <a href="%s#more-%d" class="more-link">%s</a>', get_permalink( $post ), $post->ID, $rg->more_text );
    }
    // text, excerpt, content
        ...
        $more_link = $rg->more_text;
        if($rg->more_link){
            $more_link = sprintf( '<a href="%s" class="more-link">%s</a>', get_permalink( $post ), $more_link );
        }

        if( $text !== $cuted_text ){

            // del last word, it not complate in 99%
            $text = preg_replace( '/(.*)\s\S*$/s', '\\1'.$more_link, trim( $cuted_text ) );
        }
